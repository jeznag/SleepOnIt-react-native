...
Given that Pat (high S) has written the following emails:
    |Let's consider our actions carefully because we don't know what might happen. I really want to make sure everyone agrees with this. We don't want to ruffle anyone's feathers. Let's take some time to reflect on what we should do next.|
    |Let's hold off making a decision on this until we can gather more information. I want to make sure that the team is happy with the new office move. Let's all take some time to think about what we all think and come back together in a few weeks|
